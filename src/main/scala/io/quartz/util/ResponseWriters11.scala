...
////////////////////////////////////////////////////////////////////////////
  ...
  def writeBLOBtoChannel(
    ...
    val result = for {
      ...
      header <- ZIO.attempt(ResponseWriters11.genResponseContentTypeFileHeader(fpath.toString, contentType))

      fpm = ZIO.acquireRelease(attemptBlocking(new FileInputStream(fpath)))(fp =>
        ZIO.attempt(fp.close).catchAll(_ => ZIO.unit)
      )
